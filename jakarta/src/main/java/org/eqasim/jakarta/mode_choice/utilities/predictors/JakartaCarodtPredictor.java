...
public class JakartaCarodtPredictor extends CachedVariablePredictor<CarodtVariables> {
	...
	@Inject
	public JakartaCarodtPredictor(ModeParameters parameters, @Named("carodt") CostModel costModel) {
		this.costModel = costModel;
		this.parameters = parameters;
	}

	@Override
	public CarodtVariables predict(Person person, DiscreteModeChoiceTrip trip, List<? extends PlanElement> elements) {
		if (elements.size() > 1) {
		...
		return new CarodtVariables(travelTime_min, cost_MU, euclideanDistance_km, accessEgressTime_min);
	}
}
