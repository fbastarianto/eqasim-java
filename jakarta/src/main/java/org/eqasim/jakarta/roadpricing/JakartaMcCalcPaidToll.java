...
public final class JakartaMcCalcPaidToll implements LinkEnterEventHandler, VehicleEntersTrafficEventHandler, VehicleLeavesTrafficEventHandler {
	...
	final JakartaMcRoadPricingScheme scheme;
    ...
    @Inject
	JakartaMcCalcPaidToll(final Network network, final JakartaMcRoadPricingScheme scheme, EventsManager events ) {
		super();
		events.addHandler(this);
		this.network = network;
		this.scheme = scheme;
		if (JakartaMcRoadPricingScheme.TOLL_TYPE_DISTANCE.equals(scheme.getType())) {
			this.handler = new DistanceTollBehaviour(events);
			log.info("just instantiated DistanceTollBehavior") ;
		} else if (JakartaMcRoadPricingScheme.TOLL_TYPE_AREA.equals(scheme.getType())) {
		...
		} else if (JakartaMcRoadPricingScheme.TOLL_TYPE_CORDON.equals(scheme.getType())) {
		...
		} else if (JakartaMcRoadPricingScheme.TOLL_TYPE_LINK.equals(scheme.getType())) {
...
//	}
	...
	class DistanceTollBehaviour implements TollBehaviourI {
		...
		public void handleEvent(final LinkEnterEvent event, final Link link) {
			...
			Cost cost = JakartaMcCalcPaidToll.this.scheme.getLinkCostInfo(link.getId(),
					event.getTime(), driverId, event.getVehicleId());
			if (cost != null) {
				double newToll = link.getLength() * cost.amount;
				AgentTollInfo info = JakartaMcCalcPaidToll.this.agents.get(driverId);
				if (info == null) {
					...
					/* Register it. */
					JakartaMcCalcPaidToll.this.agents.put(driverId, info);
				}
				info.toll += newToll;
	...
	class LinkTollBehaviour implements TollBehaviourI {
		...
		public void handleEvent(final LinkEnterEvent event, final Link link) {
			...
			Cost cost = JakartaMcCalcPaidToll.this.scheme.getLinkCostInfo(link.getId(), event.getTime(), driverId, event.getVehicleId() );
			if (cost != null) {
