 ...
 * @author mrieser
 */
public final class JakartaMcRoadPricingWriterXMLv1 extends MatsimXmlWriter {
	// needs to be public. kai, sep'14

	private final JakartaMcRoadPricingScheme scheme;

	public JakartaMcRoadPricingWriterXMLv1(final JakartaMcRoadPricingScheme scheme) {
		this.scheme = scheme;
	}
	...
	private void write() throws UncheckedIOException {
		try {
			...
			// cost
			if (JakartaMcRoadPricingScheme.TOLL_TYPE_DISTANCE.equals(this.scheme.getType())) {
				this.writer.write("\t<!-- amount: [monetary unit] / [link length unit] -->\n");
			} else if (this.scheme.getType() == JakartaMcRoadPricingScheme.TOLL_TYPE_AREA) {
				this.writer.write("\t<!-- amount: [monetary unit] / [simulation] -->\n");
			} else if (this.scheme.getType() == JakartaMcRoadPricingScheme.TOLL_TYPE_CORDON) {
