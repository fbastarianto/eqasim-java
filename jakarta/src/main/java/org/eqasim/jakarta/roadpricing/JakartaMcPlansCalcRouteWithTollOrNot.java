...
import java.util.Map;
 ...
 class JakartaMcPlansCalcRouteWithTollOrNot implements PlanAlgorithm {
	...
	private void replaceCarModeWithTolledCarMode(Plan plan) {
		for (PlanElement planElement : plan.getPlanElements()) {
			if (planElement instanceof Leg) {
				if (((Leg) planElement).getMode().equals(TransportMode.motorcycle)) {
					((Leg) planElement).setMode(MOTORCYCLE_WITH_PAYED_AREA_TOLL);
				}
			}
		}
	}
	...
	private void replaceTolledCarModeWithCarMode(Plan plan) {
		for (PlanElement planElement : plan.getPlanElements()) {
			if (planElement instanceof Leg) {
				if (((Leg) planElement).getMode().equals(MOTORCYCLE_WITH_PAYED_AREA_TOLL)) {
					((Leg) planElement).setMode("motorcycle");
				}
			}
		}
	}
